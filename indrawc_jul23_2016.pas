...
var
  ...
  db,lep,kesl,tkezdo,tbef,t1k,t1b,rth:integer;
  db2,db3,tlep,kesl2,ugr,tkezdo2,tbef2,ylep,telso,tutso:integer;
  nwtyp,nrtyp:integer;
  c,megjel,arnyek,bgcol:char;
  tim:string[5];
  inp:text;
  xw,yw,xr,yr:^vectc;
...
begin
  ...
  writeln;
  writeln('Choose a background color (w/b)');
  writeln;
  repeat
    gotoxy(1,wherey-1); clreol;
    readln(bgcol);
  until (bgcol='w') or (bgcol='b');
  writeln;
  if bgcol='w' then
    begin
      bgcolor:=1;
...
begin
  ...
  writeln('2. Antigen process');
  writeln('3. Antigen figure');
  writeln('4. Peptide process');
  writeln('5. Peptide figure');
  writeln;
  writeln('Choice:');
  repeat
  ...
  if szul=1 then
    begin
      if arnyek='y' then
        begin
          setcolor(green);
          circle(70+x^[ind^[i]] div 2,240+y^[ind^[i]] div 2,2);
  ...
  if szul=1 then
    begin
      if arnyek = 'y' then
        begin
          setfillstyle(1,green);
          FillEllipse(70+xt^[indt^[i]] div 2,240+yt^[indt^[i]] div 2,rth div 2,
            rth div 2);
        end
      else
        begin
          setcolor(green);
          circle(70+xt^[indt^[i]] div 2,240+yt^[indt^[i]] div 2,rth div 2);
        end;
    end
  else
    begin
  ...
  if szul=1 then
    begin
      if arnyek='y' then
        begin
          setcolor(green);
          circle(70+xt^[indt^[i]] div 2,240+yt^[indt^[i]] div 2,2);
...
begin
      ...
      i:=1;
      while (i<=db3) and (tht1^[indt^[i]]<tkezdo) do
        begin
          if tht2^[indt^[i]]>=tkezdo then
            vizsgalth(1);
          inc(i);
        end;
